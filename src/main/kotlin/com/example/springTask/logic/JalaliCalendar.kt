...
class JalaliCalendar : Calendar {
    ...
    override fun computeTime() {
        if (!isTimeSet && !isTimeSeted) {
            ...
            cal[yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, internalGet(HOUR_OF_DAY), internalGet(MINUTE)] = internalGet(SECOND)
            time = cal.timeInMillis
        ...
        when (field) {
            ...
            HOUR_OF_DAY, HOUR, MINUTE, SECOND, MILLISECOND, ZONE_OFFSET, DST_OFFSET -> {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) &&
                    ...
                    cal!![yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, internalGet(HOUR_OF_DAY), internalGet(MINUTE)] = internalGet(SECOND)
                    cal!![field] = value
                    yearMonthDate = gregorianToJalali(YearMonthDate(cal!![YEAR], cal!![MONTH], cal!![DATE]))
    ...
    override fun add(field: Int, amount: Int) {
        ...
        if (field == MONTH) {
            ...
            val gc: Calendar = GregorianCalendar(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date,
    ...
    companion object {
        var gregorianDaysInMonth = intArrayOf(31, 28, 31, 30, 31,
                30, 31, 31, 30, 31, 30, 31)
        var jalaliDaysInMonth = intArrayOf(31, 31, 31, 31, 31, 31,
                30, 30, 30, 30, 30, 29)
        ...
        fun gregorianToJalali(gregorian: YearMonthDate): YearMonthDate {
            ...
            gregorianDayNo = (365 * gregorian.year + Math.floor((gregorian.year + 3) / 4.toDouble()).toInt()
        ...
        fun jalaliToGregorian(jalali: YearMonthDate): YearMonthDate {
            ...
            jalaliDayNo = 365 * jalali.year + (jalali.year / 33) * 8 + Math.floor((jalali.year % 33 + 3) / 4.toDouble()).toInt()
            i = 0
        ...
        fun dayOfWeek(yearMonthDate: YearMonthDate): Int {
            val cal: Calendar = GregorianCalendar(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date)
